...
const canChargeBattery = useSelector(service, (state) =>
  state.can({ type: 'CHARGED_BATTERY', amount: 0 }),
)
...
<template>
  <v-card width="100%" class="ma-2">
    <template #text>
      <p>Engine is {{ isRunning ? 'running (' + speed + ')' : 'stopped' }}</p>
      ...
      <v-btn
        class="ma-2"
        @mousedown="service.send('START_BUTTON_PRESSED')"
      ...
      <v-btn
        class="ma-2"
        :disabled="!canAddFuel"
        @click="service.send({ type: 'FUEL_ADDED', amount: 15 })"
        >Add fuel</v-btn
      >
      <v-btn
        class="ma-2"
        :disabled="!canChargeBattery"
        @click="service.send({ type: 'CHARGED_BATTERY', amount: 15 })"
        >Charge battery</v-btn
      >
    </template>
  </v-card>
</template>
